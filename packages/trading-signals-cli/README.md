...
`input` reports what the indicator was fed: whole candles, or one value per bar taken from that field. Each indicator declares which part of a candle it consumes, so the command feeds exactly that. A value series is a plain number per bar and nothing in the library says which series it is, so the handful of indicators that mean volume rather than price (`vroc`, `pvo`, `rvol`) are named here and fed volumes. `required` is the number of inputs it needs before it emits anything, and `result` is `null` while it is still warming up. `signal` appears for indicators that derive a trend from their result.
