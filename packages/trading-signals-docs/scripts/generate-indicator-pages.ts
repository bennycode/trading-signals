...
`
  ...
  sidebarSections.push({items, text: categoryTitle});

  // Category overview; it also forwards the previous site's `/indicators/<category>/#<id>` links.
  const descriptions = new Map(categories[category].map(config => [config.name, config.description]));
  const overview = `---
title: ${JSON.stringify(categoryTitle)}
---

import {LegacyHashRedirect} from '../../../components/LegacyHashRedirect';

<LegacyHashRedirect category=${JSON.stringify(category)} ids={${JSON.stringify(categories[category].map(config => config.id))}} />

# ${categoryTitle}

${items.map(({link, text}) => `- [${text}](${link})${descriptions.get(text) === text ? '' : `: ${descriptions.get(text)}`}`).join('\n')}
`;
  fs.writeFileSync(path.join(pagesDir, 'index.mdx'), overview);
}
