...
4. **Implement the contract** — declare `inputShape` (the compiler derives the one correct literal from the `Input` generic, so a mismatch fails to compile; `MovingAverage` subclasses inherit it); `getRequiredInputs()` returns the warm-up count; `update(input, replace)` returns the result or `null` during warm-up. `replace()` must be correct: if the result is a pure function of the candle window, `pushUpdate()` handles it; if the indicator carries smoothing state, restore the previous state on replace.
