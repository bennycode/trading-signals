...
function PerformanceCards({candles, result}: ResultProps) {
  ...
  return (
    <div className="grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-3">
      {cards.map(card => (
        <div key={card.label} className="demo-card p-3 min-w-0">
          <div className="flex items-center gap-1">
            <div className="text-xs demo-muted">{card.label}</div>
            <div className="relative group">
              <div className="demo-muted hover:opacity-80 cursor-default text-xs leading-none">ⓘ</div>
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 w-48 demo-card rounded px-2.5 py-1.5 text-xs demo-text hidden group-hover:block z-10 shadow-lg">
                {card.info}
                <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-(--demo-border)" />
              </div>
            </div>
          </div>
          <div
            className={`text-lg font-bold mt-1 break-words ${
              card.positive === null ? 'demo-heading' : card.positive ? 'text-green-500' : 'text-red-500'
            }`}>
            {card.value}
          </div>
        </div>
...
function TradeHistoryTable({result}: {result: BacktestResult}) {
  if (result.trades.length === 0) {
    return (
      <div className="demo-card">
        <h3 className="text-base font-semibold demo-heading mb-3">Trade History</h3>
        <p className="demo-muted text-sm">No trades were executed.</p>
      </div>
    );
  }

  return (
    <div className="demo-card overflow-x-auto">
      <h3 className="text-base font-semibold demo-heading mb-3">Trade History</h3>
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b demo-divider">
            <th className="text-left demo-muted py-2 px-3">#</th>
            <th className="text-left demo-muted py-2 px-3">Date</th>
            <th className="text-left demo-muted py-2 px-3">Order</th>
            <th className="text-right demo-muted py-2 px-3">Price</th>
            <th className="text-right demo-muted py-2 px-3">Size</th>
            <th className="text-right demo-muted py-2 px-3">Total</th>
            <th className="text-right demo-muted py-2 px-3">Fee</th>
          </tr>
        </thead>
        <tbody>
          {result.trades.map((trade, i) => (
            <tr key={i} className="border-b demo-divider hover:bg-(--demo-accent)/5">
              <td className="py-2 px-3 demo-text">{i + 1}</td>
              <td className="py-2 px-3 demo-text font-mono text-xs">{formatDate(trade.openTimeInISO)}</td>
              <td className="py-2 px-3">
                <span
                  className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${
                    trade.side === OrderSide.BUY ? 'bg-green-500/15 text-green-500' : 'bg-red-500/15 text-red-500'
                  }`}>
...
function WinnerBadge() {
  return (
    <span className="ml-2 inline-block bg-yellow-500/15 text-yellow-600 border border-yellow-500/40 text-xs font-semibold px-2 py-0.5 rounded-full">
      Winner
    </span>
  );
}
