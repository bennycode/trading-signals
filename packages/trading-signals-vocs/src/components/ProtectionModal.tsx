  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4" onClick={onClose}>
      <div className="demo-card p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
        ...
        {hint && <p className="text-xs text-amber-600 mt-3">{hint}</p>}

        <div className="flex gap-2 mt-6">
          <button
            onClick={handleRemove}
            className="flex-1 py-2 px-3 bg-(--demo-accent) hover:opacity-90 text-white rounded-md text-sm cursor-pointer transition-colors">
            Remove
          </button>
          <button
            onClick={onClose}
...
function GuardSection({guard, onChange, suffix, title}: GuardSectionProps) {
  ...
  return (
    <div className="demo-card p-3">
      <label className="flex items-center gap-2 text-sm demo-heading cursor-pointer">
        <input
          type="checkbox"
      ...
      {guard.enabled && (
        <div className="mt-3 space-y-3">
          <div>
            <label className="block text-xs demo-muted mb-1">Trigger type</label>
            <select
              value={guard.triggerType}
              onChange={e => update({triggerType: toTriggerType(e.target.value)})}
              className="w-full px-2 py-1.5 demo-card demo-text rounded text-sm focus:outline-none focus:border-purple-500">
          ...
          <div>
            <label className="block text-xs demo-muted mb-1">{valueLabel}</label>
            <input
              type="text"
              value={guard.value}
          ...
          <div>
            <label className="block text-xs demo-muted mb-1">Order type</label>
            <select
