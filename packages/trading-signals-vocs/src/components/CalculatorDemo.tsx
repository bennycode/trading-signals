    ...
    <DemoCard name={example.name} description={example.description}>
      ...
      <div>
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-xs demo-muted mb-1">{example.inputs[0].label}</label>
            <input
              type="number"
              value={a}
              onChange={event => setA(event.target.value)}
              className="w-full px-3 py-2 demo-card demo-text text-sm focus:outline-none focus:border-blue-500"
            />
          </div>
          <div>
            <label className="block text-xs demo-muted mb-1">{example.inputs[1].label}</label>
            <input
              type="number"
              value={b}
        ...
        <div className="demo-card mb-4">
          <div className="text-xs demo-muted mb-1">{example.outputLabel}:</div>
          {error ? (
            <div className="text-lg font-mono text-red-400">{error}</div>
          ) : (
            <div className="text-2xl font-mono font-bold text-green-400">
              {result}
              {example.outputSuffix && <span className="demo-muted text-base ml-1">{example.outputSuffix}</span>}
            </div>
          )}
        </div>
