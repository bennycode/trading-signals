...
- A broker without market data accepts an optional `marketData?: MarketDataSource` in its constructor and delegates candle calls to it. The reference pairing is Trading212 (broker) + Alpaca (`MarketDataSource`); Alpaca already streams 1-minute bars over WebSocket, so wiring a separate provider is unnecessary for US instruments. Lifecycle of the data source is owned by the *caller*, not the broker — `disconnect()` on the broker does not close it.
