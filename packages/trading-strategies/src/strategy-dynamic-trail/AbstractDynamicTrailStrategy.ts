...
export abstract class AbstractDynamicTrailStrategy extends Strategy {
  ...
  /**
   * Warm-up hook (called by the runtime on session start). Pulls enough historical candles at the
   * ATR timeframe to stabilize the ATR, so the trail is volatility-sized from the very first live
   * candle instead of sitting unprotected through the warm-up period. Only the ATR is seeded — the
   * peak still starts at attach, since there is no position to guard until then.
   */
  override async init(market: Pick<MarketDataSource, 'getRecentCandles'>, pair: TradingPair): Promise<void> {
    ...
    const candles = await market.getRecentCandles(pair, count, intervalInMillis);
