...
describe('REI', () => {
  ...
  describe('replace', () => {
    it('replaces the most recently added value', () => {
      ...
      rei.replace({close: 3_000, high: 4_000, low: 2_000});
      expect(rei.getResultOrThrow().toFixed(2)).toBe(replacedResult);

      rei.replace(testData[testData.length - 1]);
      expect(rei.getResultOrThrow().toFixed(2)).toBe(result);
    });
  });
...
describe('FasterREI', () => {
  ...
  describe('replace', () => {
    it('replaces the most recently added value', () => {
      ...
      rei.updates(testData);
      expect(rei.getResultOrThrow().toFixed(2)).toBe(result.toFixed(2));

      rei.replace({close: 3_000, high: 4_000, low: 2_000});
      expect(rei.getResultOrThrow().toFixed(2)).toBe(replacedResult.toFixed(2));
  ...
  describe('getResultOrThrow', () => {
    ...
    it('detects neutral momentum', () => {
      const rei = new FasterREI(8);
      const inputs: HighLowClose<number>[] = Array(rei.getRequiredInputs()).fill({
        close: 180,
        high: 180,
        low: 180,
      });
  ...
  describe('comparison with Big.js implementation', () => {
    it('produces results equivalent to the Big.js version', () => {
      const bigREI = new REI(8);
      const fasterREI = new FasterREI(8);

      // Generate identical candles for both implementations
      for (const candle of testData) {
        bigREI.add(candle);
        fasterREI.add(candle);
      }

      // The faster version should produce results close to the Big.js version
    ...
    it('handles edge cases the same way', () => {
      const bigREI = new REI(8);
      const fasterREI = new FasterREI(8);

      // Test with identical low volatility data
      const lowVolatilityCandles: HighLowClose[] = Array(bigREI.getRequiredInputs() + 5).fill({
        close: 100,
        high: 100,
        low: 100,
      });
      ...
      for (const candle of lowVolatilityCandles) {
        bigREI.add(candle);
        fasterREI.add(candle as HighLowClose<number>);
      }

      expect(fasterREI.getResultOrThrow()).toBe(0);
      expect(bigREI.getResultOrThrow().toNumber()).toBe(0);
    });
  });
});
