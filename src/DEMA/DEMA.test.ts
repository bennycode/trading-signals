...
import {DEMA as NativeDEMA} from '../../native';
...
describe('DEMA', () => {
  describe('getResult', () => {
    it('calculates the DEMA with interval 10', () => {
      ...
      const nativeDEMA = new NativeDEMA(10);

      prices.forEach((price, index) => {
        dema.update(price);
        fasterDEMA.update(price);
        nativeDEMA.update(price);
        if (dema.isStable) {
